...
def main():
    ...
    entropy = 0
    for k in Serotypes_count_dict.keys():
        entropy -= ((Serotypes_count_dict[k] / denominator) * (math.log(Serotypes_count_dict[k] / denominator, 10)))

    print(entropy)

    f = open(odir + "/selected_no_of_feats_sero", "w")

    f.write("number_of_fea" + "\t" + "Accuracy" + "\n")

    if entropy <= 1:
        with open(impt_dir + "/impt_vs_cv_accuracy_sero_level.txt", "r") as fh:
            arr = []
            for line in fh:
                # print(line)
                line = line.rstrip().split("\t")
                arr.append("{0:.3f}".format(float(line[1])))
        max_value = max(arr)  # maximum value
        max_keys = arr.index(max_value)  # getting all keys containing the `maximum`

        f.write(str(10 + 10 * max_keys) + "\t" + str(max_value) + "\n")
