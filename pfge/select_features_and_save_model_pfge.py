...
#############################Getting PFGE Mapping and Serotype Mapping into dictionary###########################################
    ...
    feats_dict = {}
    with open(idir2+"/selected_no_of_feats", "r") as fh:
        next(fh)
        for line in fh:
            # print(line)
    ...
    for k,v in feats_dict.items():
        ...
        for file in l2:
            if sra2sero[file] == k:
                ...
                kmers = subprocess.run(
                    mash + " info -d " + idir + "/" + file + ".msh" + " | grep -P '^\s+[0-9]+' | sed -r 's/\s+//g' | sed 's/,//'",
                    shell=True, stdout=subprocess.PIPE, encoding='utf-8').stdout.rstrip().split("\n")
        ...
        #labelencoder_file = open(odir+"/label_encoder_%s" % k, "wb")
        ...
        selected_fea = importances.index[0:int(v)]
        X_fea = X_model.loc[:, selected_fea]
        # print(X_fea)

        features = list(X_fea.columns)
        with open(odir+"/selected_features_%s.txt" % k, "w") as f1:
            for i in features:
                f1.write(str(i) + "\t")

        clfs2 = RandomForestClassifier(n_estimators=500)
        clfs2.fit(X_fea, Y_model)
        pickle.dump(clfs2, open(odir+"/final_model_%s" % k, "wb"))
